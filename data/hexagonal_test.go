...
func TestHexagonal(t *testing.T) {
	...
	for i := 3; i <= 20; i++ {
		...
		data := HexagonalData(float64(i), 1)
		...
		if data.Metric != 'e' && data.Metric != 'p' {
		...
		for _, edge := range data.Edges {
			if math.Abs(data.DistanceSquared(data.Vertices[edge[0]], data.Vertices[edge[1]])-math.Pow(data.VV, 2.0)) > TestEps {
				t.Errorf("%d Inner product between %v and %v doesn't equal %f \n <a,b> = %f", i, data.Vertices[edge[0]], data.Vertices[edge[1]], math.Pow(data.VV, 2.0), data.DistanceSquared(data.Vertices[edge[0]], data.Vertices[edge[1]]))
			}
		}
