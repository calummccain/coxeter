...
func TestTetrahedron(t *testing.T) {
	...
	for i := 3; i <= 20; i++ {
		...
		for _, vertex := range data.Vertices {
			if data.Metric == 's' {
				if math.Abs(data.InnerProduct(vertex, vertex)-1.0) > eps {
				...
				if math.Abs(data.InnerProduct(vertex, vertex)) > eps {
		...
		for _, edge := range data.Edges {
			if math.Abs(data.DistanceSquared(data.Vertices[edge[0]], data.Vertices[edge[1]])-math.Pow(data.VV, 2.0)) > eps {
		...
		if math.Abs(data.DistanceSquared(data.CFE, data.CFV)-math.Pow(math.Cos(math.Pi/data.P), 2.0)) > eps {
			t.Errorf("%d Angle between CFE and CFV is not pi/%f", i, data.P)
		}

		if math.Abs(data.DistanceSquared(data.CFE, data.FEV)) > eps {
			t.Errorf("%d Angle between CFE and FEV is not pi/2", i)
		}

		if math.Abs(data.DistanceSquared(data.CFE, data.CEV)) > eps {
			t.Errorf("%d Angle between CFE and CEV is not pi/2", i)
		}

		if math.Abs(data.DistanceSquared(data.CFV, data.CEV)-math.Pow(math.Cos(math.Pi/data.Q), 2.0)) > eps {
			t.Errorf("%d Angle between CFV and CEV is not pi/%f", i, data.Q)
		}

		if math.Abs(data.DistanceSquared(data.CFV, data.FEV)) > eps {
			t.Errorf("%d Angle between CFV and FEV is not pi/2", i)
		}

		if math.Abs(data.DistanceSquared(data.CEV, data.FEV)-math.Pow(math.Cos(math.Pi/data.R), 2.0)) > eps {
